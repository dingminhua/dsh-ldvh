...
- **全局信息层空白（Human 2026-09-06 指出，须记录并补充设计）**：现有分层=规范层（全局一份，管规则）+ 事实层（每项目各自，管项目事实）——**缺一档「所有管辖项目都要知道的事」**。空白四块：
  ①**框架级通知**：LDVH 框架自身的重大变更（如 0.1.2 升级调查结论）须让每个管辖项目的主控知道——现在只存在于 dsh-ldvh 仓库的 docs/ 里，其他项目根本读不到；
  ②**跨项目经验**：对一切项目适用的坑/经验（宿主 API 变更类），现在只能重复塞进每个项目的事实源（N 份副本+更新不同步）；
  ③**跨项目协调**：多项目并行时的共享信息（分发/复用/共同依赖）；
  ④**Web 呈现维度缺失**：Web 侧只有单项目视角，须增加「所有管辖项目」维度（跨项目总览——项目清单/各项目状态/全局信息的呈现入口）。
  **定位初判**：非规范（不是规则）、非项目事实（非单项目所有）——第三种全局层；候选载体=管辖实例级目录（governed-projects.yaml 所在 ~/.dsh/ldvh/ 下，登记文件已含 governance_instance_name 全局实例概念但零全局信息载体）；读写纪律沿写维模型（受控写入+回读），但跨项目写入的分寸（谁能写/如何传播到各项目会话）待设计。**去向：08 修订批次新增议题或沉淀维串联站裁决**（与热记忆候选同批考虑——两者都动「信息在哪、谁可读」的结构）。
...
### 8.3 事实对象边界定调（Human 2026-09-06，串联中段新增——事实模型第一性原则）

**核心命题：LDVH 是生产系统，不是证据系统**——一切记录为结果服务。六条：

1. **人可读性约束**：事实对象 = Human 能看得过来的内容——体量上限由人的阅读带宽定义，超出者不进对象（过程细节的归宿是事件流/会话记录，不是事实对象）；
2. **证据为结果服务**：留证支撑「下一步做什么」与「出事可查」，不为留存考据而全录；**可溯源=线索级**（顺藤能摸到），不是细节级；
3. **WC 过程态/最终态分离**：最终态（判据结果+结论）进对象；过程态不进（v4 WC 的签名 change_log 逐条流水=过程态混入对象的反例）；
4. **Spark 增加临时/议题承载职能**：不止 v4 的分流器——「不值得立正式对象、但活在事实源有上下文价值」的议题记录由 Spark 承载；
5. **事实对象外延收拢**：LDVH 内除外部文档（上游资料/第三方调研等）外，其余文档最终都应是事实对象——现存 docs/ 的调查报告/工作稿（升级调查/各草案）不得成为「永久的非对象常驻文档」：要么升格、要么临时态（Spark/工作稿机制），最终收敛；
6. **v4 的 310 份是警示不是资产**（WC 157/Study 53/Spark 94，附录 B #26 记录性质就此改判）：Study 均 160+ 行、WC 八判据逐条核对——超出人的可读带宽，LDVH 反着来。

**对已定设计的三处校准**：①执行用法遗留 9 同向但不足——补「最终态对象体量的人可读性闸」（频率分级只管了更新节奏）；②复核维「轮次留档审计链」按此校准——**结论封存保留（线索级）**，全量过程留档过度；③附录 B #26 性质改判如上。

**去向**：03（事实模型）修订批次的定纲原则；沉淀维串联站执行分流表校准。
