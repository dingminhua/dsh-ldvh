...
插件设置面板承载管辖项目登记管理、固定登记载体位置说明与 LDVH Web 呈现控制；Web 服务复用宿主 `webServer`（前缀路由 `/ldvh` 与 `/ldvh/api`），零独立端口：

- 显示：Web 路由启用状态、实际访问地址（`http://<host>:<webServer 端口>/ldvh/`）、路由注册状态、最近失败原因；
- 管辖项目登记/取消/默认项目按 07 规范原子写与回读；
- 登记载体固定为 DSH 用户配置根下 `ldvh/governed-projects.yaml`（经正式路径 API 解析，不硬编码），与通用设置同属用户配置体系但使用独立文件；设置页只说明相对位置，不允许修改，也不展示实际绝对路径；
- 操作：关闭 / 开启 LDVH Web 呈现（即时注销/注册路由）；开启后自动检查最终可用状态，不提供重复的检测或重启按钮；
- 配置错误、路由冲突和页面不可达的区别提示；
- 默认行为是插件加载时注册路由，Human 可通过开关关闭或重新开启。
