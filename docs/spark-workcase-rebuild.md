...
| 2 | ~~evolution 上限~~／~~priority 字段去留~~ | 20 | **priority 部分已关闭**（2026-09-13）：v5 两侧均不引入 priority。依据 20 §289（v4 priority 不迁入）、21 §8（WorkCase 字段闭集无此项）、22 §271／23 §252／26 §258（priority 类为无消费方装饰字段——重要性由消费点引用体现）、03 §11.3-4（字段扩展须先说明消费方）。两类型消费点表均无按优先级排序者，且 20／21 召回分层明文声明「被召回不表示可以提高其优先级」。Web 侧残留（列表筛选、卡片徽标、`?priority=` 参数、`priorityOptions` 投影）已同步移除，见 `docs/web-workcase-v4-residue-census.md`。**evolution 上限部分仍开放。** |
