...
| 13 | 2026-09-08 | （补记·要义）「架构简化」——砍除 Initiative 层，sub-goal 直承分解链（commit 0940b81） | **Initiative 退役**：类型收敛六个（21 WC / 20 Spark / 25 Goal / 22 ADR / 23 Pitfall / 24 Research），WC `serves` 直挂 SG-n；蓝图六区块收敛五区块（ed15b8b）；26 号编号空出（后由 Friction 摩擦账本类型复用，2026-09-10 另线） |
| 14 | 2026-09-09 | （补记·要义）计划并入 goal（目标+计划一体），蓝图不再有独立文件（commit 7614f7b） | **蓝图= 纯计算视图**：五区块全部计算生成、零 authored（§5.3 当时形态）；goal.md 增计划区/方法区——2026-09-10 Human 决定移除两区（「方法与规划后续重新设计」悬空），成为裁定 15 直接前因 |
| 15 | 2026-09-10 | 「直接写到 goal 里，我觉得就可以；关键是目标的那一句话，必须是授权才能改写，而不能 ai 自己写，如果不能控制，那就需要 2 个文档了」→ 经主控可行性核对（Git Gate 区域分类器可建、fail-closed），Human 选定**四项全冻** | **蓝图形态定案（§5.3 改写）**：authored 区（下一步/演进脉络）内嵌 goal.md，翻转裁定 14「纯计算视图」；冻结区闭集四项（目标陈述/子目标 SG-n/frontmatter 身份与状态/模板结构）仅 Human 授权（C1 + gate 标记独立提交）；authored 区 AI 判断维护（CAS+change_log）；**机械前提：Git Gate 冻结区分类器先于 authored 区上线，不上线不开放（否则退两文档）** |

---

## 5. 事实对象架构定稿（v2，Human 基本认可 2026-09-08）

> 认可口径（Human 原话引，裁定 10 时代）：「一个锚（goal）、一条分解链、一间候诊室（Spark）、一个知识库（ADR/Pitfall/Research）、一张规划面（蓝图）。」**裁定 13 更新**：分解链= sub-goal→WC（两层），类型= 六个（21 WC / 20 Spark / 25 Goal / 22 ADR / 23 Pitfall / 24 Research）。**裁定 14 更新**：计划并入 goal（目标+计划一体），蓝图= 纯计算视图（无文件载体）——六类型 + 零特殊载体。**裁定 15 更新**：蓝图= 投影区块 + goal.md 内嵌 authored 区（下一步/演进脉络）——「零特殊载体」字面维持（无第二个文件），「纯计算视图」翻转（authored 区由 AI 判断维护、分区保护，§5.3）。
...
### 5.2 事实对象名册（六件；裁定 15 后蓝图仍无独立载体——authored 区内嵌 goal.md）
...
| **goal.md** | 25 | 前向·锚 | 目标陈述 + G-SC-x（冻结锚点）+ status + authored 区（裁定 15） | active→achieved（=项目收官，唯一翻转） | **单例** | 冻结区 Human Gate / authored 区 AI 判断维护 | Gate 1 链顶；蓝图「当前目标」与 authored 区投影源；讨论三件套之一 |
...
### 5.3 蓝图（投影 + goal.md 内嵌 authored 区，裁定 15）

裁定 14 曾定「纯计算视图、零 authored 内容」，但其「下一步」唯一投影源 goal.md 计划区已于 2026-09-10 经 Human 决定移除（「方法与规划后续重新设计」悬空），纯投影也给不出判断性内容（下一步的理由、演进因果线——HV5 核心）。**裁定 15 翻转：蓝图 = Web 投影区块 + goal.md 内嵌 authored 区**：

| 区块 | 来源 |
|---|---|
| 当前目标 | goal.md 目标陈述 + SG-n 达成概览（投影） |
| 下一步 | goal.md authored 区（AI 判断维护） |
| 演进脉络 | goal.md authored 区（追加式——从哪来、为何拐弯；裁定 15 新增，区别于裁定 13 砍掉的「当前子目标」：前者承载因果叙述，后者与「进行中」数据源重复） |
| 已完成 | closed WC 关闭链（按 SG-n 分组，投影） |
| 进行中 | open WC（按 SG-n 分组，投影） |
| 悬而未决 | open Spark（投影） |

goal.md 分区（裁定 15）：**冻结区闭集四项**——目标陈述、子目标（SG-n 行，原位作废是唯一例外）、frontmatter 身份与状态字段（goal_key/title/status/created_at）、模板结构（标题/节序/增删 H2 节）——仅 Human 授权可改（C1 提案 + 确认 + gate 标记独立提交，提交只含 goal.md）；**authored 区**（下一步/演进脉络）+ change_log 追加由 AI 判断时机与内容（CAS + change_log，普通受控提交）。

机械前提（Human 条件的工程化）：**Git Gate 冻结区分类器先于 authored 区上线**——fail-closed 四规则：冻结区 diff 无 gate 标记拒收；change_log 非追加（改/删旧条目）拒收；标题/节序/模板结构变动按冻结区处理；解析失败拒收。检查不上线，authored 区不开放，方案退回两文档（goal.md 纯锚 + 蓝图叙述单例）。诚实边界：Gate 读不到对话，gate 标记证明「提交声明过授权且被迫单独成提交、可 grep」，最后一公里靠 25 号 §12「修订授权」审计行——与 LDVH 全部关卡同一信任模型。

红线：仍不是第二事实源——已完成/进行中/悬而未决严禁手抄事实源（保持机械投影）；authored 区只写投影写不出的判断性内容；字面上仍无第二个文件（「零特殊载体」维持）。
...
| Feature | sub-goal 的产品语义特例 | 产品项目里 feature≈SG-n；外部流水线工件（Spec Kit specs/ 目录等）经端点契约收割后挂靠对应 sub-goal，不做副本。**注（2026-09-14）**：本行是新设计输入，尚无规范或代码承载——「feature = sub-goal 产品语义特例」这一定义本身在 04/21/26 三号均零命中，端点契约契位亦未选型；由 Spark `629ab3a5` 承载 |
...
5. **「快通道」是风险轴的专用词，不得用于工件规模轴**（2026-09-14 登记）：04 §6.1 已把「快通道」定义为**风险预检分层三档之一**（快通道/标准路径/加固路径，三档互斥、系统必须择一、不得另立第四层），§6.3 定其**环节级**升级规则，该词已进入 04 positioning 字段，是 04 号的唯一权威用词。而本总纲 §3.3、§9 借 keel 吸收的「规模变体」（微/常/大三档模板，「同门禁不同工件」）属**工件规模轴**——其对立面是工件大小，与风险高低无关（一个高风险环节可以有很小的工件，反之亦然）。**二者是正交的两轴，不是一个词的两义**。后果警示：若 21 号引入「小工作准入」机制时把「微」等同于「低风险」，将直接违反 04 §6.1 的三档互斥纪律与 §6.3 的环节级升级规则（高风险环节可借「工件小」逃避标准路径预检）。**执行要求**：涉及工件规模轴时不得简写为「快通道」，须写全「规模变体档」或另定不撞车的新词；引用风险轴时保留「快通道」原义。登记于术语表（`specs/attachments/01.Att.01-LDVH双语术语表.md`）的对应条目随 25/26 号起草批次连带；**本条的最终措辞（双轴分名 vs 加限定语）由 Spark `5b47f9ec` 裁决**。
...
**落点列的读法（2026-09-14 校正，重要）**：原「落点」列未区分目标层，导致按单一判据（在 `specs/` 中检索）核对必然产生大面积假阴性——凡落实现层者一律显示为「未落」，凡依 §8.4 判据①「吸模式不吸功能」正确不落者亦显示为「未落」。现按三层分列，**每层的核对判据不同**：

- **`[规]` 规范条文**——落点须在 `specs/` 正文可检索；
- **`[实]` 实现层**——落点在 `plugin/lib/`、`plugin/web/` 或 Git Gate 代码，判据是**行为可观察**（在 `specs/` 中检索必然为零，检索无结果**不构成**未落的证据）；
- **`[不]` 判据排除**——依 §8.4 判据①明确不落（落既有机制实现层，不新增独立机制）；此类「未落」是判据被正确执行的结果，**非缺口**。

| # | 机制 | 来源 | 落点（分层） | 落点（具体位置） |
|---|---|---|---|---|
| S1★ | Complexity Tracking 表（违反原则须填表自证无更简方案） | Spec Kit | **[不]** | 已由 00 §6.3「答不出的不纳入」+ 01 §15 最小充分修改 + 04 §13 反过度设计红线以**判据取代工件**；落它反属违反 §8.4 判据① |
| S2 | clarify 十类歧义分类+可答形状约束（≤5 问/多选/短答） | Spec Kit | [规]/[实] | 11 号 + 实现 |
| S3 | 未知项→研究子任务转换 | Spec Kit | [规] | 11/编排 |
| S4 | 任务-需求覆盖映射（零覆盖即高危，可机械检查） | Spec Kit | **[实]** | ✅ 已落 `plugin/lib/commit-validation.js` 双向覆盖检查（`staged_path_uncovered` + `EXEMPT_BASENAMES`，06 §6.2 具名） |
| S5★ | feature↔WorkCase 对齐（端点契约=生态对齐点） | Spec Kit | **[规]** | ❌ **未落**（真缺口）：`specs/` 与代码均零命中；见 §11 开放问题第 7 条，另由 Spark `629ab3a5` 承载 |
| C1★ | requireHuman 四层机械门（AI 提案对象模式） | spec-collab | [规] | 08/21 |
| C2★ | commit 钉扎确认+受影响范围自动失效 | spec-collab | [规] | 21 |
| C3 | FACT 强证据机械约束（accessible+versioned 否则 throw） | spec-collab | [规] | 11 |
| F1★ | tasks.md 复选框任务级进度（记录+进度+选择器三合一） | specflow | [规] | 21/04（勾选形态未落，见 Spark `15006169`） |
| F2 | 单一发现入口+goal 前缀反推对象 | specflow | [规] | 08 |
| F3 | goal 桥三态恢复 | specflow | [规] | 08/02 |
| F4 | open question 必须显式留人、不静默决定 | specflow | [规] | 11/00 |
| K1★ | 规则 ID 稳定引用体系（finding{rule,severity,line}） | keel | **[实]** | ✅ 已落 `plugin/lib/commit-validation.js:49-58`——10 条规则的冻结注册表 `GATE_RULES`，`newFinding()` 产 `{rule, severity, message, line}`，未注册即抛错（fail-loud） |
| K2★ | 同门禁更小工件的模板规模变体（微/常/大三档） | keel | **[实]** | 原标「04」易读作规范号；**§9 自述落「04 实现层」**（「微/常/大三档，同门禁小工件」），即实现层，非规范条文 |
| K3 | 共享核心+双壳（插件 apply 与裸 CLI 复用，检查面进 CI） | keel | [规] | 08 |
| L1 | 标准事件折叠状态机（运行态投影，零自定义事件） | spec-loop | [实] | 02 实现层 |
| L2 | 门与显示同源投影（呈现读行为门同一载体） | spec-loop | **[实]** | ✅ 已落 `plugin/lib/web-preferences.js:9` 与 `plugin/web/shared/projectColors.ts` 的双闭集同步约束 |
| L3 | 声明式 bash 验证命令+有界 judge 成本闸门 | spec-loop | **[不]** | 已由 04 §5.4「验证入口」+ 09 §102 测试基线（拒绝零测试实现）以更强形态取代；且 judge 成本闸门对应外部 CLI 计费约束，LDVH 无此约束 |

**编号变更提示（2026-09-14）**：本表「落点」列中的 **26** 指当时尚未砍除的 Initiative（26 号），该层已由裁定 13 退役，**26 号编号其后由 Friction 摩擦账本复用**（2026-09-10 另线，见 §4 裁定 13）。故凡本列出现「26」者（如原 S5「21/26」）不得读作现 26 号 Friction。
...
立即可做（不依赖类型重建）：
  ⓪ 蓝图=投影 + goal.md authored 区（裁定 15；顺序硬依赖：Git Gate 冻结区分类器 → 25/06/10 规范修订批次 → Web FocusV2 接线 authored 区渲染）
...
| 2 | ~~evolution 上限~~／~~priority 字段去留~~ | 20 | **priority 部分已裁定**（2026-09-13，Human 推翻当日早前的关闭结论）：**Spark 引入 `priority`**（闭集 `P0`–`P3`，AI 综合判断出初值、Human 可按情况调整，`open` 时存在、终态省略、允许缺失），**WorkCase 不引入**（维持 21 §8 字段闭集无此项）。已落 `specs/20` §8/§9.1/§12/§15/§289 与 Web 三联过滤（`priority`+`serves`+`status`）。**演化记录（如实登记）**：同日早些时候曾据 20 §289 与 22 §271 等将其记为「已关闭、两侧均不引入」，该结论**由 Human 裁定推翻**——其论证以被质疑的规范证明规范，属循环，不作先例保留。现依据为 Human 决定权（00 §4）与本字段的消费点登记（10 §5.2、20 §12）。**evolution 上限部分仍开放。** |
| 3 | Spark→Research 分流判据（含 Spark→goal 修订的新分流路径） | 20/24 | 开放 |
| 4 | attempt 令牌 vs 指纹 CAS | 21/08 | 开放 |
| 5 | 快通道准入判据（「小工作」的定义；keel 规模变体判据参考） | 21/04 | 开放 —— **附注（2026-09-14）**：本条措辞含轴混装，见 Spark `5b47f9ec`。「快通道」在 04 §6.1 是风险预检分层的专用词，而 keel 规模变体属工件规模轴，两轴正交；执行时须先按 §5.9 第 5 条区分，不得把「微」等同于「低风险」 |
| 6 | 授权包字段集瘦身（八组件→最小充分集） | 21 | 开放 |
| 7 | 端点契约五契位选型 + 收割产物挂靠 sub-goal 的具体格式 | 21 | 开放 —— **附注（2026-09-14）**：经核实（§8.1 S5 行）本条所载设计输入在规范、代码与事实源三处承载全为零，是 §8.1 中唯一确证的规范缺口，由 Spark `629ab3a5` 承载 |
