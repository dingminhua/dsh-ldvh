...
| D4 | **从 00 重构** | 新 00 基于 v4 `specs/00` 原文重构（经 Human 同意修订，2026-08-24）：§1 两处——结尾定位句改「充分利用 DeepSeek Harness 开源的优势，深度绑定宿主机制」、工具清单句去 DSH；§2 方案区重组——AI 诊断行补「和多视角复核」、统领句「对 AI 执行者实施受控调度」、第 5 条改「DSH 原生插件承载 AI 编排执行/Git Gate/Web」、新增「同样的治理效果，更低的摩擦」目标句、目标 1 Helper CLI→Helper 服务、目标 3 外部验证→核对。其余按五维重新组织；00 保持薄，只定义「根 + 接口 + 原则」，字段/状态机/模板细节全部下沉到随搬运重建的下位规范 |
