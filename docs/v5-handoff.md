...
| D3 | **开源属性不变** | MIT License（已在新仓库根目录），可进 DSH 插件市场（dshfind），接受开源公共契约 |
...
│                         Git Gate 终闸 / 受控写入边界 / Host Hook 与插件事件
│                         （turn-end 锁恢复、pre-step 引导时机）/ 单调令牌与冷恢复
├─ §5 技术吸收原则       ← 新增：
...
五维不是拍脑袋，每维精确对治 00 §1 的一个根缺陷：

| 00 §1 根缺陷 | 五维 | 对治机制（LDVH 已有） |
|---|---|---|
| AI 上下文依赖（不知道缺什么、用推断补全） | **读** | 渐进读取：规则引导 → F0 → F1/F2 候选 → 影响判断才展开 F3/F4 |
| AI 无连续状态（前说后忘、重复犯错） | **写** | 受控写入：事实对象 + 指纹 CAS + 跨会话回读 |
| AI 概率性输出（会错、会幻觉、单次成功不可外推） | **思考** | 判断分层：相关/适用/授权/完成分开 + V/HV 监督 + 独立复核 |
| Human 进展断续（任务难以接续） | **行动** | 行动单元 + 现取模板 + 验证 + 交还 |
| AI-Human 摩擦（事前设卡、过程透明、事后溯源） | **执行** | 三通道：Skill 路由 / Git Gate 把关 / Web 呈现 |
...
- 受控写入只能经 Helper；Git Gate 仍是最终闸门；Skill 保持环境无关且逐字节一致
...
2. **起草新 00**（进行中）：落点规范源根 `specs/00-理念与构成.md`；§1/§2 已逐字锁定 v4 原文（见 D4），逐章讨论从 §3 起，Human 逐章过目
...
- **09 §5.1 冲突**：插件模式打破「唯一接入单元：薄 Skill」，新 00 必须重新定义「插件管能力（机械半边）/ Skill 管思维（语义半边）」的分工
