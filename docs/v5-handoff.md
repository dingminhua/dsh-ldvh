...
| D4 | **从 00 重构** | 新 00 的 §1 主体/§2 **逐字保留 v4 `specs/00` 原文**（经 Human 同意修订，2026-08-24：§1 两处——结尾定位句改「充分利用 DeepSeek Harness 开源的优势，深度绑定宿主机制」、工具清单句去 DSH；§2 一处——AI 诊断行补「和多视角复核」），其余按五维重新组织；00 保持薄，只定义「根 + 接口 + 原则」，字段/状态机/模板细节全部下沉到随搬运重建的下位规范 |
...
├─ §2 解决方案          ← §2 主体逐字保留 v4 00 §2 原文；AI 诊断行经 Human 同意修订（补「和多视角复核」，2026-08-24）：三要素（规范/事实/模板）
