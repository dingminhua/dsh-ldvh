...
- `specs/01-规范模型基础规范.md`：定稿（编号终表 §6.1：01 规范模型 / 02 六维工作模型 / 03 事实模型 / 04 行动模板 / 05 Helper 服务 / 06 事实源与信息溯源 / 07 工作对象与管辖 / 08 环境接入 / 09 Code / 10 Web；20–24 事实类型段；30–38 行动模板段；附件 `<父编号>.Att.<序号>`）。
...
把「内部关联五环节必决问题清单」写入 `docs/dev-memo.md` 待定项 11，作为 03 事实模型起草的检查表：
...
### 步骤 2：起草 02 六维工作模型规范（单篇，先做职责地图）

六维各占一章（读/写/遵守/审查/执行/检讨），每章只定：职责、判据、关键机制指向（指向 03/04/05/06/20–24/30–38 等）——机制细节不复制进本文；某维长大后按最小充分修改拆分，不预先拆分。Git Gate 与权限等属于横向机械守护，不重复归入单一维。02 先给 03 的技术路由和职责分工提供边界，不把六维变为六套流程。

### 步骤 3：起草 03 事实模型基础规范

要点（草案须覆盖）：五类事实（ADR/Pitfall/Spark/WorkCase/Study，编号段 20–24 的公共层）的公共字段与载体契约；受控创建/更新/读取（指纹 CAS、写后回读、适用完整性审计）；**关系声明五环节**（按步骤 1 清单落章）；`dimensions` 声明；与 00 §5.2 事实源边界、§3.2 受控创建与入档/事实预热注入、§3.3 三层防线、§4.3 证据纪律的承接。触达待定项 14–16（生命周期改进、Spark 决定、命名）时**只给方案呈 Human 定，不自行决定**。起草后按纪律 6 走双审核 + Human 审 + 受控提交（`docs(specs):`）。

### 步骤 4：起草 04 行动模板基础规范

要点：行动模板五机制（触发与排除契约、步骤停止条件、反逃避信号、完成门禁、分层评测）；快通道（00 §5.5 风险分层，快通道由行动模板定义、层级数/判断标准/声明方式统一定义）；预检落实记录（遵守维机械承载，格式由本文定义——00 §5.5.1 落点）；按 02 的职责地图采用已验证宿主技术与最小充分降级路径，不把技术清单写成固定仪式；与 00 §3.2/§5.4/§5.5 承接。
...
按 01 §6.1 登记表逐一注册起草；先 20/21/22/23/24（事实类型，与 03 联动），后 30–38（行动模板实例，与 04 联动）。
