...
| 机械 | dsh-industry-research | 三态编码+SHA-256 证据链+交付合同+确定性多视角+版本账本 | 25 Investigation 字段契约、来源/证据与关系契约、版本账本 |
...
| # | 来源 | 机制 | 吸到 | 备注 |
|---|---|---|---|---|
| 1 | deep-research | 三态证据 `confirmed`/`uncertain`/`gaps` Schema | 25 §8 字段契约 + 11 §7 三态证据 | 必填结构化字段，含置信度 |
...
| 7 | industry-research | SHA-256 证据哈希链 + 版本账本（artifact 写入即追加） | 25 §10 来源与证据 + 06 提交契约 | 哈希由 Code 托管，AI 不可填 |
| 8 | industry-research | 确定性多视角（正反方 bull/bear）+ 交付合同（不全成即失败） | 11 §10 交付合同 + 25 §13 受控操作 | 多视角属 Helper 确定性动作 |
...
| industry-research | `industry_map` / `industry_track` / `company_scan` 行业领域工具 | 是 Investigation 的一个领域示例，不是 11/25 的通用契约 |
...
| socrates | 报告自动生成最终 Markdown | 11 不规定"出报告"产物形态；25 不规定 Investigation 必须渲染为报告 |
...
| 吸收项 | 11 调研系统 | 25 Investigation 类型 | 实现层 |
|---|---|---|---|
