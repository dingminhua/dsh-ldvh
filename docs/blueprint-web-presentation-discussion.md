...
> 讨论发起：Human（2026-09-08，审批策略改为 never 后；结合调研对象 0ca1ab2c 内容讨论蓝图在 LDVH 的呈现。原稿误记 2026-09-11，该日晚于本文 commit 60973d7 不可能成立，订正）
> 状态：蓝图形态已裁定（**裁定 15**，2026-09-10，权威记录见 spark-workcase-rebuild §4/§5.3）——authored 区内嵌 goal.md + 四项冻结闭集 + Git Gate 检查先行；D1/D5 与更新触发仍开放。代码未动。
...
- **spec 10 增「蓝图投影格式」章节**（25 已声明由其承载，目前空洞）——裁定 15 后形态已定：读 goal.md（陈述/SG-n 投影 + authored 区渲染）+ 事实源聚合（WC/Spark）；
- ~~蓝图是否受保护内容（草案 §5 问题 2）~~ → **已裁定（裁定 15，2026-09-10）**：goal.md 分区保护——冻结区闭集四项（目标陈述/子目标/身份状态字段/模板结构）走 gate 标记独立提交，authored 区走普通受控提交；
- 蓝图 authored 区更新触发（原草案 §5 问题 3：机械 vs 空闲梳理）——空闲评审代理尚未建成（记忆系统批次，reflection seam 仍是空骨架），倾向机械节点（WC 关闭/受控提交后主动提议）先行、空闲触发后补，待正式裁决；
...
| 日期 | 事件 |
|---|---|
| 2026-09-08 | Human 结合调研对象 0ca1ab2c 提出讨论蓝图呈现；本稿记录现状核对 + 三个方向决策 + 待决点 D1–D5（原稿误记 2026-09-11，按 commit 60973d7 订正） |
| 2026-09-10 | **裁定 15**：蓝图形态定案——authored 区（下一步/演进脉络）内嵌 goal.md；冻结区闭集四项（目标陈述/子目标/身份状态/模板结构）仅 Human 授权；Git Gate 冻结区分类器先于 authored 区上线（不上线退两文档）；翻转裁定 14「纯计算视图」。D1/D5 与更新触发仍开放（见 §4） |
