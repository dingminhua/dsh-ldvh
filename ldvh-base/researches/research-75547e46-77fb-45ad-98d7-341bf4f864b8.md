...
confirmed:
  - statement: Linear 的 Initiative/Project update 由健康指示灯+富文本描述两部分固定结构组成
    confidence: high
    quotes:
  ...
  - statement: Linear 的健康指示灯枚举值是 On track / At risk / Off track 三档
    ...
    quotes:
      - text: Select a health indicator— On track, At risk, or Off track—to represent the current state of the Initiative or Project.
        ...
        source: https://linear.app/docs/initiative-and-project-updates
  - statement: GitHub Projects 的 status update 固定结构为：状态+开始日期+目标日期+Markdown 消息
    confidence: high
    quotes:
  ...
  - statement: GitHub Projects 的 roadmap 布局在可配置时间跨度上展示条目并以自定义日期/迭代字段定位
    confidence: high
    quotes:
  ...
  - statement: Jira Plans 的 Summary 视图用于跨团队与计划的顶层进度快照
    confidence: high
    quotes:
  ...
  - statement: Jira Plans 在 epic 之上支持自定义层级 Initiative 作为跨团队史诗容器
    confidence: high
    quotes:
  ...
  - statement: GitHub Projects 内置工作流可在条目被添加或变更时自动设置字段
    ...
    quotes:
      - text: Built-in workflows allow you to automatically set fields when items are added or changed, and you can also configure your project to automatically archive items when they meet certain criteria
        ...
        source: https://docs.github.com/en/issues/planning-and-tracking-with-projects/learning-about-projects/about-projects
  - statement: Linear 通过周期性提醒督促 owner/lead 持续发布 update，且仅发给 In Progress 状态的对象
    confidence: high
    quotes:
      - text: Reminders are sent for active Initiatives and Projects with statuses in the In Progress category, and only to the Initiative owner or project lead.
        ...
        source: https://linear.app/docs/initiative-and-project-updates
  - statement: Linear 自动生成进度报告且总体进度变化超过 2% 才显示细节
    confidence: high
    quotes:
      - text: Overall project progress must have changed by more than 2% since the last update in order for progress details to appear here.
        ...
        source: https://linear.app/docs/initiative-and-project-updates
  - statement: Linear 提供 Write with Agent：agent 审查变更与 Slack 消息起草 update 草稿供人细化发布
    confidence: high
    quotes:
      - text: click **Write with Agent** to let Linear do it for you. The agent reviews changes made since the last update, checks messages in the linked Slack channel, and writes an update draft for you to refine.
        ...
        source: https://linear.app/docs/initiative-and-project-updates
  - statement: Jira Plans 以沙箱方式工作：改动停留在 plan 内直到 Review changes 提交
    confidence: high
    quotes:
      - text: No. Plans works as a sandbox. Any changes you make stay in your plan until you choose to save them back to Jira using the "Review changes" function.
        ...
        source: https://www.atlassian.com/software/jira/guides/advanced-roadmaps/overview
  - statement: Jira 的 Program Board 从 plan 实时拉取数据保持最新，是活数据而非快照
    confidence: high
    quotes:
      - text: Because it pulls from your plan in real time, the board stays current as work progresses.
        ...
        source: https://www.atlassian.com/software/jira/guides/advanced-roadmaps/overview
  - statement: Linear 最新 update 展示在概览页，历史通过 Updates 标签页查看
    confidence: high
    quotes:
      - text: The most recent update is displayed on the Project Overview or Initiative Overview page. To view previous updates, click on the Updates tab.
        ...
        source: https://linear.app/docs/initiative-and-project-updates
  - statement: GitHub 的 status update 位于项目侧栏，最新在最上方、历史在其下排列
    confidence: high
    quotes:
      - text: Status updates can be found on your project's side panel, below the description and README. You can read the most recent update at the top and the full history of updates beneath.
        anchor: About status updates
        source: https://docs.github.com/en/issues/planning-and-tracking-with-projects/sharing-project-updates
  - statement: GitHub 新建 status update 表单默认沿用上一条的状态与日期，降低重复填报
    confidence: high
    quotes:
  ...
  - statement: Linear 的 update 可在 Linear 内查看并推送到 Slack 频道
    ...
    quotes:
      - text: Updates can be viewed in Linear and sent to Slack channels.
        anchor: Overview
        source: https://linear.app/docs/initiative-and-project-updates
  - statement: Linear 的 update 支持 emoji 反应，且变更记录与 update 按时间顺序同列
    confidence: medium
    quotes:
...
gaps:
  ...
  - description: Notion/Airtable 项目 dashboard 模板无实际抓取来源
    priority: low
implications:
  - finding_ref: Linear 的 Initiative/Project update 由健康指示灯+富文本描述两部分固定结构组成
    implication: 业界 update 的固定结构支持 LDVH 蓝图采用固定区块——但六区块本身无业界先例（LDVH 原创），可维护性是蓝图设计的首要风险
  - finding_ref: GitHub Projects 的数据随 issue/PR 变更自动双向同步进项目，视图与图表无需手动更新
    implication: GitHub 的数据自动双向同步提示 LDVH 蓝图应从事实源机械投影而非手写维护——更新触发器可与 Git Gate/事实源变更联动
  - finding_ref: Linear 自动生成进度报告且总体进度变化超过 2% 才显示细节
    implication: Linear 的 2% 变化阈值可防止蓝图更新噪声——但 LDVH 是 AI 主导维护，阈值语义需按 LDVH 场景重定义
  - finding_ref: Linear 提供 Write with Agent：agent 审查变更与 Slack 消息起草 update 草稿供人细化发布
    implication: Write with Agent（agent 起草+人细化）与 LDVH 蓝图的 AI 维护+Human 验收分工同构，可直接吸收
  - finding_ref: Jira Plans 以沙箱方式工作：改动停留在 plan 内直到 Review changes 提交
    implication: Jira 沙箱+Review changes 与 LDVH 蓝图受控更新（CAS+change_log）同构——业界先例支持蓝图走受控通道而非自由编辑
...
## 未证实与缺口
- 缺口（medium）：业界无与 LDVH 六区块蓝图相同的固定区块先例——三家均为健康状态+自由文本；六区块是 LDVH 原创设计，其可维护性无外部参照，是蓝图实现的首要风险
- 缺口（medium）：Linear Initiatives 独立文档页未抓取正文
- 缺口（medium）：Jira 单项目 roadmap 页面未抓取正文验证
...
对 LDVH 蓝图设计的启发：
- 「Linear 的 Initiative/Project update 由健康指示灯+富文本描述两部分固定结构组成」——业界 update 的固定结构支持 LDVH 蓝图采用固定区块——但六区块本身无业界先例（LDVH 原创），可维护性是蓝图设计的首要风险
- 「GitHub Projects 的数据随 issue/PR 变更自动双向同步进项目，视图与图表无需手动更新」——GitHub 的数据自动双向同步提示 LDVH 蓝图应从事实源机械投影而非手写维护——更新触发器可与 Git Gate/事实源变更联动
- 「Linear 自动生成进度报告且总体进度变化超过 2% 才显示细节」——Linear 的 2% 变化阈值可防止蓝图更新噪声——但 LDVH 是 AI 主导维护，阈值语义需按 LDVH 场景重定义
- 「Linear 提供 Write with Agent：agent 审查变更与 Slack 消息起草 update 草稿供人细化发布」——Write with Agent（agent 起草+人细化）与 LDVH 蓝图的 AI 维护+Human 验收分工同构，可直接吸收
- 「Jira Plans 以沙箱方式工作：改动停留在 plan 内直到 Review changes 提交」——Jira 沙箱+Review changes 与 LDVH 蓝图受控更新（CAS+change_log）同构——业界先例支持蓝图走受控通道而非自由编辑
