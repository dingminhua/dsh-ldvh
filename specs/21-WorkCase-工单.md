...
- **去向词为闭集五词**：「另立工单」「补录」「更正」「接受现状」「改进」——写去向，不写自由短语。
...
**为什么需要（实测理由）**：`status = open` 时 `result` 字段尚不存在，故「待批准关闭」期的处置建议同样只在正文里、无任何词表约束。实测 5 份结果节共 8 条建议，全部以「建议…」子句**嵌在 `residual` 条目的正文里**：其中 3 条因所在条目无加粗标题而丢失出处；另有一条被 `；` 截断（原文「…独立复核建议改为 fail-closed；属另立单范围。」只抽出「建议改为 fail-closed；」）。呈现层只能靠关键词从句中抓取，并已实测**误判一例**——「建议后续清理时把……另立单对象号……一并登记」的去向是**补录**，却因正文含「另立」二字被判为「另立工单」。固定建议段后，建议的存在、去向与出处均可机械判定。本条同属写法约定、**非机械门禁**（理由与上一条同）；**本条只约束建议段，不约束其它段落对该词的引用**。
