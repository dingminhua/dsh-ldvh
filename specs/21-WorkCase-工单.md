...
**本项两项的承载边界（如实登记）**：「剩余责任的去向」逐条落在正文「## 结果」节的建议段（`§8` 已登记其形态）——但**该项自身无机械校验**（`§15.1` 未列建议段校验项；`§8` 亦声明该条为写法约定、非机械门禁），故本项为 Gate 2 的**提请内容要求**，由 AI 层与 Human 层核对，**不声称已被机械覆盖**。「关闭后的后续方向」为**整单**一条，**当前无专用字段**——`result` 字段闭集只有 `criteria_checks`／`achieved_scope`／`residual` 三样，故它只能以正文叙述承载，呈现层不单独读取；此处同样如实声明。
