...
| `summary` | string | 必填 | 要做什么的当前语义快照（AI 向） | 正文「摘要」段逐字包含；使未读原计划的后续执行者可独立执行；只承载当前仍适用的范围，不复制讨论过程；**不设长度上限**——它是给执行者的完整快照，可含编号引用、对象标识与 Markdown 标记；Human 扫读面不消费本字段（见下方「`gist` 与 `summary` 的分工」） |
| `scope` | string | 必填 | 授权范围与边界 | 正文「授权范围」段逐字包含；必须同时回答「做什么」与「明确不做什么」；是越权拒绝的比对基准 |
...
```
（H1 标题行 = title）
## 摘要        ← summary 逐字 + 展开；要做什么（必填）
## 授权范围    ← scope 逐字 + 展开：做什么 / 明确不做什么（必填）
## 计划        ← plan 的正文承载（必填）：步骤 + 逐条完成判据；各 `step` 按数组序出现
## 执行        ← attempt 与执行进展的正文承载；跨会话接力（条件出现）
## 结果        ← result 的正文承载：逐条判据核对 + 已证实/未证实/残留 + 残留处置建议；`outcome = cancelled` 时为取消记录（关闭提案时必填）
```
...
**「复核」的记账纪律**：复核的**发起**与**结论录入**（含派发独立复核、委派复核子代理、回传复核结论）时，须经受控更新写入 `change_log`，其摘要以显式形式「**复核：**」**开头**（例：「复核：步骤 1–3 完成…；复核已发起」）。本条不发明语义，只规定一个**可机械识别的前缀**：上条已把「复核」明文列为非计划步骤的执行事项，「## 执行」节即其正文承载，故该写入是**真实的内容修改**，属 03 §9.5 应记之列，不是过程日志、工具调用或 no-op。

**为什么需要前缀（实测理由）**：`change_log` 中「**复核这件事**」与「**处置复核的发现**」共用同一批词，字面无法区分。实测三例：「独立复核（两视角）的 12 条发现与处置」是**复核报告**，「复核 2 项必须处置全部落地」是**修订动作**，「复核与处置非计划步骤」是**记账更正**——三者都含「复核」，语义却分属三类。靠关键词识别必然误判；故以「复核：」前缀把**复核条目**显式标出。前缀是**唯一合法写法**，本条不为其设机械门禁（写成其它形式即不可检出），其可遵守性依靠工具层在写入时提示（见 §14 受控操作），**不依靠检查**。

**「复核中」的判据由此可得，但其两侧可靠性不同（如实登记）**：`change_log` 存在前缀条目 ∧ `reviews` 不存在 ⇒ 复核在途（10 §5.5）。其中**完成一侧是机械的**——`record_review` 由 Code 盖戳写入 `reviews` 并在 `change_log` 留 `[review recorded by session …]` 标记，不依赖作者；**发起一侧依赖本条前缀**。故「复核中」是「机械完成标记 + 前缀发起标记」的组合判据，**不得据本条声明复核过程已被完整机械记录**。

**「格式治理」的记账纪律**：与当次执行无关、仅调整本对象文本形态（如摘要分块、空白归一）的写入，其 `change_log` 摘要须以「**格式治理：**」开头。此类条目**不构成该对象的执行进展或复核后修订**，呈现层判定阶段时须予排除（10 §5.5 口径 ③）。本条同属写法约定、非机械门禁——理由与前两条同。

**「结果」节的核对结论词表**：关闭提案正文承载（「## 结果」节，见下表 `## 结果` 行）中，逐条判据的核对结论须使用**闭集三词**之一：「**达成**」「**部分达成**」「**未达成**」，写成「判据/步骤…：<词>——证据：…」的形式。三词与 `result.criteria_checks[].satisfied` 的映射是：**「达成」⇔ `true`；「部分达成」「未达成」⇔ `false`**（后两者须在 `residual` 中有对应去向，依 §10.2）。

**为什么需要固定词表（实测理由）**：`status = open` 时 `result` 字段尚不存在（本表：「`result` 与 `outcome` 出现 ⇔ `status = closed`」），故「待批准关闭」期的核对结论**只在正文里**，无任何词表约束。实测 5 份结果节出现过三套同义写法——「达成」（3 份）、「已满足」（1 份）、「已完成」（1 份）——呈现层因此无法可靠归一，同一状态在不同对象上显示为不同词。固定三词后，机械层可判定、呈现层可归一、Human 可跨对象比较。本条同属写法约定、非机械门禁（理由与前三条同）；**本条只约束本节的核对结论词，不约束其它段落对该词的引用**（如描述三态映射的正文）。

**「结果」节的建议去向词表与建议段**：关闭提案正文承载（「## 结果」节，见下表 `## 结果` 行）中，对残留的处置建议写成**独立的建议段**：

```
- advice:
  - **<去向词>**：<建议正文>出自「<所回应的 residual 条目标识>」
```

- **去向词为闭集四词**：「**另立工单**」「**接受现状**」「**转入 Spark**」「**直接行动**」——写去向，不写自由短语。四词的含义以**责任去哪**为准，**不以动作内容分**：「另立工单」＝转为新建的 WorkCase 承接；「接受现状」＝明确不跟踪，就此了结；「转入 Spark」＝转为 Spark 悬置议题待裁；「直接行动」＝不另立对象，由本单或其后续批次直接处置（补录、更正、改进、清理一类动作均归此词）。
- **一行一条**：去向词以 `**` 包裹并位于行首，后接 `：`；建议正文写该去向要做的动作，**不得以重复去向词充当正文**（去向词已写在行首）。
- **建议段整行为纯文本**：正文与 `出自「…」` 均不使用 Markdown 强调或行内代码标记——卡面按纯文本渲染（与 `gist` 同纪律），写了标记就只是把标记本身显示给读者。出处里若要引用标识符，直接写其字面即可。
- **`出自「…」` 为可选尾注**：写其所回应的 `residual` 条目，须紧接建议正文的句末标点之后、不带句号；条目无标题时写足以辨认该条目的文字。
- **存在建议段时，`residual` 条目内不得再写「建议…」子句**——处置建议只有一处正文承载，避免同一建议被计两次。
- **事后补记声明**（Human 授权 2026-09-24）：若建议段是在对象**关闭之后**补写的（如 2026-09-23 建议段登记前已关闭的存量对象），段**前**须有一个独立段落，形如 `**本条为 YYYY-MM-DD 事后补记，非关闭当时的 Gate 2 提请内容。**`。**该声明必须随建议段一同呈现在卡面**——它的用途正是让读者知道这段去向不是关闭时的提请输入；若只留在详情面，卡面读者仍会以为它当初被提请过，声明就失去意义。声明由呈现层按**固定写法**识别（不取「建议段前的任意段落」——实测宽判据会把标题、清单行、正文段落一律当声明）。本条属写法约定，**非机械门禁**。
- **建议段的性质是「提请时如实说明的打算」，不是承诺**（Human 裁定 2026-09-24）：Gate 2 批准的对象只是**关闭**与 `outcome`（`§10.2`），建议段不因关闭而转为已获批准的动作。故呈现层**不得以暗示「已批准」的措辞**呈现它——包括但不限于「后续去向」一类乐观表述；关闭后若继续呈现，用中性表述（如「去向」）。**该界线的完整说明见 `§10.2`。**

**为什么需要（实测理由）**：`status = open` 时 `result` 字段尚不存在，故「待批准关闭」期的处置建议同样只在正文里、无任何词表约束。实测 5 份结果节共 8 条建议，全部以「建议…」子句**嵌在 `residual` 条目的正文里**：其中 3 条因所在条目无加粗标题而丢失出处；另有一条被 `；` 截断（原文「…独立复核建议改为 fail-closed；属另立单范围。」只抽出「建议改为 fail-closed；」）。呈现层只能靠关键词从句中抓取，并已实测**误判一例**——「建议后续清理时把……另立单对象号……一并登记」的责任去向是**直接行动**（不另立对象、由后续批次处置），却因正文含「另立」二字被判为「另立工单」。固定建议段后，建议的存在、去向与出处均可机械判定。本条同属写法约定、**非机械门禁**（理由与上一条同）；**本条只约束建议段，不约束其它段落对该词的引用**。

**为什么是四词而不是更多（Human 裁定 2026-09-24，实测依据）**：词表原先取五词（另立工单／补录／更正／接受现状／改进），实测暴露它**混了两种分类维度**——「另立工单」「接受现状」按**责任去哪**分，而「补录」「更正」「改进」按**动作内容**分。后果有二：① 同一维度被拆成三个词，而它们其实是同一去向（都不另立对象、都由本单或后续批次直接处置）；② `§10.2` 的 Gate 2 提请必含项另举三值（转入 Spark 悬置／转新建 WorkCase／明确不跟踪），两套词表**互不覆盖**——五词缺「转入 Spark」，三值缺「直接行动」。四词按**单一维度（责任去哪）**取值，恰好补全两处缺位。**实测校验**：存量 8 条建议逐条按实际语义重映射为四词，**8/8 全部有归宿、无遗漏无歧义**（另立工单 4 条；直接行动 4 条——原「更正」1、「改进」2、「补录」1 均归此词）。

**「结果」节的取消记录（仅 `outcome = cancelled` 时）**：计划未经执行即被取消的对象，其「## 结果」节写成**取消记录**，而不是逐条判据核对（没有执行就没有核对结论可写）：

```
- cancellation:
  - **理由**：<为什么取消>
  - **未发生的范围**：<哪些计划步骤或工作从未发生>
```

- **两行均必填且非空**——§9.2 与 §9.3 已明文要求「记录取消理由与未发生的范围」（§9.2：「`draft → closed`（`outcome = cancelled`）：计划未经执行即被明确取消，Human Gate；`result` 记录取消理由与未发生的范围」；§9.3：`cancelled`「记录取消理由与实际未发生的范围；不得以 cancelled 掩盖已完成或已失败的事实」）。本条只给该要求一个**可机械识别的形态**，不新增义务；
- **取消记录存在时，本节不得再写逐条判据核对**（没有执行就没有核对结论）；
- **取消记录不进 `result` 字段闭集**——`result.achieved_scope` 仍按 §8 原义承载「已证实范围」（cancelled 时可写「未执行，无已证实范围」一类如实陈述），取消理由本身由本节承载，**避免再次把两件不同的事塞进同一个字段**。

**为什么需要（实测理由）**：本仓唯一一份 cancelled 对象（`workcase-af430278`）把两件事**合并写进了 `achieved_scope`**——一个语义为「已证实范围」（即「做成了什么」）的字段里装着「**未发生的范围**」，而取消恰恰意味着什么都没做。它靠作者自觉使用「取消理由：…未发生的范围：…」的行内标签才可读；而机械层对该字段只校验**非空**，故「本工单取消」四字同样通过写入。呈现层因此无法可靠取到取消理由，卡面只能整段不显示——规范已明文要求的内容，在卡上完全不可见。本条同属写法约定，但其两行**同时是机械门禁**（见 §15.1：字段非空是存在性校验，与 `gist` 的长度上限同层），理由与「建议段」不同：建议段无门禁是因为其内容正确性不可机械判定，而「两行是否写了」是可判定的。
...
字段间不变量：**载体内聚**（`summary`/`scope` 在正文对应段逐字出现，`plan[].step` 在「计划」段按数组序出现；frontmatter 是唯一权威文本，正文只展开。机械校验，见 §15.1 与 §16）；**书写结构**（`scope` 两个标签各占一行、内容在其下；`summary`/`scope` 超过 200 字符时须空行分块且首块之外的每块以 `### 标题` 开头——`summary` 只认 H3，`scope` 另认其登记的两标签行。机械校验，见 §15.1 与 §16）；`plan` 每项必须有非空 `done_criteria`；**`gist` 在 `status ∈ {draft, open}` 时必填且 ≤ 200 字符，在 `status = closed` 时条件（缺失合法）**；`gate_1` 出现 ⇔ `status ∈ {open, closed}`；`attempt` 出现 ⇔ `status = open`；`result` 与 `outcome` 出现 ⇔ `status = closed`；`result.criteria_checks` 必须与 `plan` 逐条对应（长度一致、顺序一致）；`outcome = completed` 时每条 `criteria_checks` 必须判为达成，否则 `outcome` 只能是 `partial`；`outcome = partial` 或 `not-achieved` 时 `residual` 必须非空；`serves` 出现时必须匹配 goal.md 中存在的 SG-n；`reviews` 每项 `summary` 必须非空且 ≤ 600 字符、条数 ≤ 20；**`status = closed` 时 `reviews` 必须非空**（关闭前须已存在至少一条独立复核记录，§9.1/§14）；且 `reviews` 与 `change_log` 分立承担（前者记复核节点结论，后者记变更流水），互不替代。未知字段处理：按 03 §6.1，未知字段不进入 canonical 对象，不得以空字段或占位代替判断。
...
Gate 2 把执行结果与残留责任提交 Human 关闭。提请必含：`result.criteria_checks` 的逐条核对结论与各自依据；已证实范围、未证实范围与残留风险；建议的 `outcome` 与理由；剩余责任的去向（**逐条**给出，词表取 `§8` 登记的**闭集四词**——另立工单／接受现状／转入 Spark／直接行动；本项原先只在此处以「转入 Spark 悬置、转新建 WorkCase 或明确不跟踪」举例，与 `§8` 的词表互不覆盖，2026-09-24 改为一处权威）；关闭后的后续方向（**整单**一条：本工单关闭后这条线往哪走；与上一项的「逐条去向」不同层，不得以前者代填）。

**批准的对象只是「关闭」与 `outcome`，不是提请里的每一项**（Human 裁定 2026-09-24：「批准的是关闭而不是认同去向，这个很重要」）：上述「提请必含」是 Human 作出判断所依据的**输入**，其中只有 `outcome` 是被判定、被落盘的对象，关闭翻转也只针对这件事。**其余各项（含「剩余责任的去向」与「关闭后的后续方向」）不因关闭而被批准、也不因关闭而成为承诺**——它们是**提请时如实说明的打算**，其是否执行、何时执行，仍属后续判断。

**为什么必须写明这条界线（实测理由）**：本条原先只列了「提请必含」，没有说明**哪些属于批准对象**。呈现层据此把「去向」当作已获批准的承诺处理，并计划在关闭后改称「后续去向」——那会让读者以为这些动作**已被授权**；一旦未执行，就会被误读为「违背承诺」。而 §9.3 亦未把去向纳入任何成立条件。**界线不明时，呈现与阅读双方都会越界**：去向该显示，但**不得以任何暗示「已批准」的措辞呈现**（如「后续去向」一类乐观表述）；关闭后若继续呈现，须用中性表述（如「去向」）。

**本项两项的承载边界（如实登记）**：「剩余责任的去向」逐条落在正文「## 结果」节的建议段（`§8` 已登记其形态）——但**该项自身无机械校验**（`§15.1` 未列建议段校验项；`§8` 亦声明该条为写法约定、非机械门禁），故本项为 Gate 2 的**提请内容要求**，由 AI 层与 Human 层核对，**不声称已被机械覆盖**。「关闭后的后续方向」为**整单**一条，**当前无专用字段**——`result` 字段闭集只有 `criteria_checks`／`achieved_scope`／`residual` 三样，故它只能以正文叙述承载，呈现层不单独读取；此处同样如实声明。**另：去向只有一处承载**（正文建议段，关闭不删正文），故关闭前后**读的是同一处**——不需要「两套」，也不得在关闭时另存一份（那会制造同一信息的第二处承载）。
...
- **创建（draft）**：C1 提案对象模式（AI 只产出提案对象，含查重结果；Human 确认后经受控创建入口落盘）。创建前必须查重。机械校验：字段闭集合法、`gist` 非空且 ≤ 200 字符、`plan` 每项 `done_criteria` 非空、`scope` 两句标签各独占一行且内容在其下（§8 书写结构）、`serves`（若声明）匹配 goal.md 存在的 SG-n、`status = draft` 且无 `gate_1`/`attempt`/`result`、载体内聚（`summary`/`scope` 逐字，`plan[].step` 按序）、书写结构（§8：`summary`/`scope` 的分块与 H3 块首）。创建后精确回读。
...
- 载体内聚：`summary`/`scope` 在正文对应段（摘要/授权范围）逐字出现（frontmatter 是唯一权威文本，正文只展开）；`plan[].step` 在「计划」段按数组序出现，防正文计划与权威数组分叉。比对为精确字符串，不做归一化。
- 书写结构（§8）：`scope` 须含**各独占一行**的「做什么：」与「明确不做什么：」两个标签行（标签行只含标签本身，可带尾随空白；冒号全角半角均可），且**内容须位于标签行之下**——即标签行到下一个标签行（或结尾）之间须有非空内容。以下均不合规：行内串联（`做什么：A；B。明确不做什么：C。`）、标签与内容同行（`做什么：A`）、标签下无内容。`summary` 与 `scope` 超过 200 字符时须以空行分块，且**首块之外**的每个块的首行须为 **H3 标题行**（`### 标题`）：`summary` 只认此一种；`scope` 另认其 §8 登记的两标签行（`做什么：`／`明确不做什么：`）——二者之外不构成块首（**2026-09-22 修订**：原文本接受 `**标签**`／`### 标签`／`标签：` 三种，改为只认 H3 及其在 `scope` 内的两个标签行）。**为何收紧（实测依据）**：三种形态在**呈现层并不等价**——`### 标签` 解析为块级元素（`<h3>` 与正文真实分隔）；`**标签**` 后的单个换行**不产生新段落**（`react-markdown` 实测仍落在同一 `<p>` 内，仅标签被加粗）；`标签：` 更无任何标记。故「三种任选」等于允许两种**读者看不出层次**的写法，与本项「结构固定清晰、有层次」的目的相悖；收敛为 H3 后机械判定与视觉层次一一对应。（标题名由作者按内容自定；本项只校验形态，不校验标题是否名副其实，也不校验内容是否真有层次——后者属 AI 语义审核与 Human 阅读，同 `gist` 完备性条的边界）。**长度阈值以内的短文本免分块**（短文本本就不难读）；但 `scope` 的两段骨架**不设长度豁免**——骨架是语义必需，与长度无关。**适用范围同 §6.1**：只校验相对基线**新增或改动**的字段，逐字未改的既有字段放行——否则存量 16/16 份（含 12 份 draft/open 活对象）连追加一条 `change_log` 都会被拒，而 `close` 是唯一出口、本类型无删除操作（§14），将使这批工单永久无法关闭。基线比对为精确字符串，不做归一化。**未验证范围**：本项只证明形态可达性，不证明标题名副其实、内容准确或结构在语义上真的清晰。
...
- 取消记录完备性（§8）：`outcome = cancelled` 时，正文「## 结果」节的取消记录须含**均非空**的「理由」与「未发生的范围」两行（形态见 §8）；缺任一行即拒绝写入。**未验证范围**：本项只校验两行存在且非空，**不校验理由是否属实、范围是否穷尽**——那是 §9.3「不得以 cancelled 掩盖已完成或已失败的事实」的语义要求，属 AI 语义审核与 Human 阅读。**存量边界**：本项只作用于受控写入，而 `closed` 是终态、无受控入口（§14），故既有 closed 对象不受影响；本仓唯一一份 cancelled 对象（`workcase-af430278`）原先把取消理由写在 `achieved_scope` 里、正文无该段，已按 Human 授权回填（2026-09-24）。
