...
- **创建（draft）**：C1 提案对象模式（AI 只产出提案对象，含查重结果；Human 确认后经受控创建入口落盘）。创建前必须查重。机械校验：字段闭集合法、`gist` 非空且 ≤ 200 字符、`plan` 每项 `done_criteria` 非空、`scope` 两句标签各独占一行且内容在其下（§8 书写结构）、`serves`（若声明）匹配 goal.md 存在的 SG-n、`status = draft` 且无 `gate_1`/`attempt`/`result`、载体内聚（`summary`/`scope` 逐字，`plan[].step` 按序）、书写结构（§8：`summary`/`scope` 的分块与层次标记）。创建后精确回读。
...
- 书写结构（§8）：`scope` 须含**各独占一行**的「做什么：」与「明确不做什么：」两个标签行（标签行只含标签本身，可带尾随空白；冒号全角半角均可），且**内容须位于标签行之下**——即标签行到下一个标签行（或结尾）之间须有非空内容。以下均不合规：行内串联（`做什么：A；B。明确不做什么：C。`）、标签与内容同行（`做什么：A`）、标签下无内容。`summary` 与 `scope` 超过 200 字符时须以空行分块，且**首块之外**的每个块的首行须为层次标记行：`**标签**`、`### 标签` 或 `标签：`（标签名由作者按内容自定；本项只校验形态，不校验标签是否名副其实，也不校验内容是否真有层次——后者属 AI 语义审核与 Human 阅读，同 `gist` 完备性条的边界）。**长度阈值以内的短文本免分块**（短文本本就不难读）；但 `scope` 的两段骨架**不设长度豁免**——骨架是语义必需，与长度无关。**适用范围同 §6.1**：只校验相对基线**新增或改动**的字段，逐字未改的既有字段放行——否则存量 16/16 份（含 12 份 draft/open 活对象）连追加一条 `change_log` 都会被拒，而 `close` 是唯一出口、本类型无删除操作（§14），将使这批工单永久无法关闭。基线比对为精确字符串，不做归一化。**未验证范围**：本项只证明形态可达性，不证明标签名副其实、内容准确或结构在语义上真的清晰。
...
| 书写结构 | 创建与更新时 | `scope` 两个标签各独占一行且内容在其下；`summary`/`scope` > 200 字符时分块且首块之外的块首为层次标记行 | frontmatter 字段文本 | 机械（行匹配 + 空行分块 + 标记行识别） | 当次字段的书写形态；不证明标签名副其实或内容准确 | 拒绝；按 §8 书写结构修复后重走 |
