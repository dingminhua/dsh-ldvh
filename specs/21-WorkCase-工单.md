...
**「复核发起」的记账纪律**：复核发起时，须经受控更新写入一条 `change_log`，其摘要以显式形式「**复核发起：**」开头，其后接当次复核的对象与视角（例：「复核发起：隔离子代理只读对抗复核，对象为本单计划步骤 1–4 的落地与证据」）。本条与上条同源分工：`plan` 的位置序号由「计划步骤 N」标识，复核的**在途状态**由「复核发起：」标识——二者使执行期各段在**不新增字段**的前提下可被读取。

**本条与上条同为写法约定，不是机械门禁**——理由同前：该形式的识别依赖作者措辞，写成「开始独立审核」等其它形式即不可检出，**字形不可判定**。故本节不为其设任何拒绝规则，也不得据此新增校验；其可遵守性依靠工具层在写入时给出该前缀提示（见 §14 受控操作），**不依靠检查**。

**本条的登记不使「复核中」成为可机械保障的状态**：它是**尽力而为的记录点**，用于呈现层读取；不得据本条声明复核过程已被机械记录或复核时点已被机械还原。
