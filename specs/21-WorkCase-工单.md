...
**「复核发起」的记账纪律**：复核发起（含派发独立复核、委派复核子代理）时，须经受控更新写入 `change_log`，其摘要须同时含「**复核**」与「**发起**」二词——既有写法「复核已发起」「独立复核已发起」即符合，**无需改写成新形式**。**本条不发明写法，只承认既有做法**：上条已把「复核」明文列为非计划步骤的执行事项，「## 执行」节即其正文承载，故该写入是**真实的内容修改**，属 03 §9.5 应记之列，不是过程日志、工具调用或 no-op。

**识别依赖措辞，本节不设机械门禁**：本条只固定「须含二词」这一最小可读形态，不规定句式，也不排除其它措辞；写成不含「复核」或「发起」的形式（如「开始独立审核」）即不可检出。故本节不为其设任何拒绝规则，也不得据此新增校验。

**「复核中」的判据由此可得，但其两侧可靠性不同（如实登记）**：`change_log` 存在此类条目 ∧ `reviews` 不存在 ⇒ 复核在途（10 §5.5）。其中**完成一侧是机械的**——`record_review` 由 Code 盖戳写入 `reviews` 并在 `change_log` 留 `[review recorded by session …]` 标记，不依赖作者；**发起一侧依赖本条措辞**。故「复核中」是「机械完成标记 + 措辞发起标记」的组合判据，**不得据本条声明复核过程已被完整机械记录**。
