...
| `question` | string | 必填 | 方向的核心待答问题 | 单句；正文「调查问题」段逐字包含；在方向完成或放弃前持续有效；允许部分已答（已答结论进 summary、结晶决定提取进 ADR，question 聚焦仍未答部分；实质变化经 evolution 记录） |
| `scope_boundary` | string | 必填 | 调查边界 | 正文「调查边界」段逐字包含；何时停止（范围、证据、时限）；非目标达成 |
| `intent` | string | 必填 | 保留理由与后续方向 | 为什么值得保留；后续需要判断的方向 |
| `summary` | string | 必填 | 完整的当前语义快照 | 正文「当前理解」段逐字包含；当前已确认判断、边界、候选方向、未决事项；使未读原聊天的后续执行者可独立理解；只承载当前仍适用的理解，不复制创建原因或过程日志 |
...
正文结构（H1「# 标题」由 Code 从 title 生成、位于围栏后首行且**恰好一个**，正文自 H2 起；此处不直书 H1 字面以免与身份 H1 重复）：

```
 # <title>
 ## 当前理解        ← summary 逐字 + 展开；完整语义快照（必填）
 ## 调查问题        ← question 逐字 + 展开；方向的待答问题与其当前已答/未答状态（必填）
 ## 调查边界        ← scope_boundary 逐字 + 展开；何时停止（必填）
 ## 演变            ← evolution 的正文承载；跨会话接力理解（条件出现）
```

字段间不变量：`question`/`scope_boundary`/`summary` **载体内聚**（正文对应段逐字包含，frontmatter 是唯一权威文本，正文只展开；机械校验，见 §14.1 与 §15）；`question` 必须单句可读；`scope_boundary` 必须明确何时停止；`intent` 必须回答「为什么保留 + 后续方向」；`summary` 必须使未读原聊天的后续执行者理解当前已知道什么、如何判断、仍需处置什么（先问当前仍适用的内容是否必须进入 `summary`，是则必须写入，`evolution` 只记变化）；`disposition` 出现 ⇔ `status ∈ {implemented, discarded}`；`priority` 仅 `status = open` 时允许出现，**终态必须省略**（与 `disposition` 互斥——二者分别标志悬置中与已终态）；`priority` 允许缺失（未分档）；`serves` 出现时必须匹配 goal.md 存在的 SG-n；`refs` 出现时每项必须是本管辖项目内可解析的事实对象 `object_uid`，不得指向外部资料（走 `urls`）、载体内部锚点（走 `serves`）或生命周期关系（走 `relations`）。未知字段处理：按 03 §6.1，未知字段不进入 canonical 对象，不得以空字段或占位代替判断。
...
| `status` | 语义 | 必须成立 |
|---|---|---|
| `open` | Spark 仍有未被稳定位置完整承接的内容，需要继续召回、判断、拆分或分流 | `disposition` 不得出现；已有关系不等于完整承接；`priority` 可出现（允许缺失） |
| `implemented` | Spark 限定的信息需求已由该 Spark 范围内的直接工作落实，或已把后续责任交给 WorkCase；只结束该 Spark 入口，不表示下游完成 | `disposition` 必填；不得有合并/拆分关系（那是 discarded 专属）；说明必须如实限定已落实或已交接的范围；**`priority` 不得出现** |
| `discarded` | 明确决定该信息不再继续跟踪；或该议题已被合并/拆分，原 Spark 不再以原形态存在 | `disposition` 必填；若声明了合并/拆分关系，目标必须可解析且为 `open`；**`priority` 不得出现** |

**`priority` 的语义边界**：`priority` 承载的是**该 Spark 在其同类悬置项中的处置次序**，用于 Human 在 Web 列表按优先级筛选、排序（10 §5.2），以及 F1/F2 投影。它**不声明对象的价值或重要性**——价值由消费点引用与实际消费体现（22 §271、23 §252、26 §258 的红线针对的是「对象自我声明价值」，本字段不在其列）。它**不是必填**：未分档的 Spark 与已分档的同等合法，缺失不构成拒绝创建或更新的理由；`priority` 缺失或非法时，排序与筛选按其缺失处理，不得代为推定档位。**取值来源**：AI 综合当前目标、`serves` 指向的子目标、`scope_boundary` 所述停止条件、被引用关系与项目当前阶段给出初值（按 03 §8.2 的语义消费边界），Human 保留按情况调整的权力；该判断**不是机械校验**，机械侧只校验闭集与状态互斥（§14.1）。

**`disposition` 的长度上限（机械校验）**：`disposition` 不超过 **200 字符**（判定单位与本规范 `title` 的同名上限一致，均为 UTF-16 码元计数；实现按 `.length` 判定）。终态原因说明是给后续读者与人扫读的定位信息，不是完整处置记录——完整理由属于正文、`change_log` 或承接对象。超限时拒绝写入（与 §11 的关系目标校验同为写入前置的机械条件，按 03 §9 的公共契约执行），不得截断后写入。

**判定的是数量而非语义**：本上限只约束长度，不判断说明是否据实、充分或与 `status` 相符——后者仍由 §15 的语义核对与 Human 处置授权承担。长度合规不证明终态处置正确。
...
**终态不可重开，但内容可更正**（Human 决定 2026-09-13）：进入终态后**状态不得再变为其它值**（不开新入口、不回到 `open`、不在 `implemented` 与 `discarded` 之间互转）；但终态对象的**内容仍可更正**——写错的、超限的、与事实不符的终态记录须就地改正，不得靠伪造一次状态转换来洗白。更正与其它受控更新同权限（Human 确认）与同纪律（03 §9.5 完整 after、指纹 CAS、恰好一条 `change_log`），且**不得借更正改变状态**。

后来出现新的未处置信息时创建新的 open Spark，并在新对象的 `disposition` 或 change_log 中说明接替的旧 Spark；这不把旧对象改写为分流。
...
### 9.4 合并/拆分的非原子例外（依 03 §9.6 明示）

03 §9.6 规定：多对象原子变化只在「当前来源已定义范围匹配的公开原子操作，且 Code 能证明全部目标、关系、基线、原子写入、失败回滚和回读」时才可声明，并**要求类型来源明示的非原子例外必须逐项定义**。本节据此为 Spark 合并/拆分定义该例外。本节以**单个原对象**为轴——每个参与处置的原对象各自走一次本节流程；多个原对象指向同一目标（多源合并）按其各自流程分别执行，整体完成声明见本节末。

**共同前提**：本类型的关系只能连同**原对象**的终态转换一起写入——受控创建不携带 `relations` 与 `disposition`（§8：二者只随终态转换附着）；关系写入时**全部目标必须已存在且为 `open`**，否则该次写入零写入拒绝（§11 去向校验、§14.1）。因此各情形各有其唯一合法次序，**目标构成**是选择次序的唯一判据。

**情形一：目标全部为既存 Spark**——**SOURCE-FIRST（原对象先落）**：

1. **第一步**：对**原对象**执行受控更新——写入终态 `discarded`、`disposition` 与指向各既存目标的关系，恰好一条 change_log；完成后精确回读；
2. **第二步**：对**每个既存目标**执行受控更新，各追加一条记录合并/拆分来源的 change_log；完成后精确回读。

理由可回指：第一步失败时原对象零写入，不产生任何假陈述；第二步失败时至多留下「目标缺一条**可幂等重补**的 additive 流水」，而**不会**出现「目标已声明来源而原对象仍为 `open`」这一假陈述。

**情形二：目标全部为新建 Spark**——**CREATE-FIRST（目标先落）**：

1. **第一步**：逐个受控创建**各新目标**（`status` 必为 `open`，不携带 `relations`/`disposition`，各自一条 change_log 记录其来源为本次拆分/合并）；每个创建后精确回读；
2. **第二步**：对**原对象**执行一次受控更新——写入终态 `discarded`、`disposition` 与指向全部新目标的关系，恰好一条 change_log；完成后精确回读。

这是机制强制的次序：若原对象先落，其关系须同时指向尚不存在的目标，直接触发零写入拒绝。此处风险方向与情形一相反——第一步失败时原对象仍完整为 `open`，**零假陈述**；第二步失败时留下「目标已建立、原对象未关闭」，该状态不产生任何假陈述（新目标如实存在且可独立处置）。

**情形三：目标混合（拆分同时指向既存 Spark 与新建 Spark）**——**先建后结**：

1. **第一步**：对**新建的那部分目标**，按情形二第一步逐个受控创建并各自精确回读；
2. **第二步**：**全部新目标建立成功后**，对**原对象**执行一次受控更新，写入终态、`disposition` 与指向**全部目标**（既存 + 新建）的关系，恰好一条 change_log；完成后精确回读；
3. **第三步**：对**每个既存目标**执行受控更新，各追加一条记录合并/拆分来源的 change_log（与情形一第二步同性质）；完成后精确回读。

混合情形下情形一的「原对象先落」不可用——原对象一旦先落，其关系就必须同时指向尚未创建的新目标，直接触发零写入拒绝。故混合情形一律适用「先建后结」。

**三种情形的共同不变量**：原对象只有**一次**终态转换与**恰好一条** change_log（不因目标数量而分裂为多次）；多目标关系由该次转换一次写入（§11：`split-into` 基数 1..n，`merged-into` 基数 1）；每个既存目标各自追加恰好一条来源流水（§13「各自追加」）。

**本例外不豁免的约束**（03 §9.6 禁止项逐条保留）：

- 不得先写孤立新对象再补关系——只在**情形二与情形三的第一步**存在「先建对象」，且这些对象随即在后续步骤被关系指向；若后续步骤未完成，须按恢复路径补做，不得使其长期停留为未被任何关系指向的**未承接**状态；
- 不得用多个单对象成功拼接整体成功——全部步骤都完成且都精确回读通过前，**不得声明合并/拆分已完成**；多源合并的每一个原对象各自完成后，才可声明该次整体合并完成；
- 不得用补偿更新制造伪原子性——不得为「回滚」而伪造中间态；失败时按下方恢复路径处置，不得把任一方改写为虚假内容；
- 失败时必须交还**完整拟变更范围、实际已完成部分、能力缺口与残留状态**，不得只报告失败一步。

**失败后的恢复路径**（按情形分别适用）：

- **情形一第二步失败**（原对象已 `discarded` 且关系有效、某既存目标流水未补）：原对象状态与关系**保持有效、不回退**；只对未补的目标**幂等重试**补写来源流水，执行前先回读确认尚未写入。**不得**把原对象改回 `open`，**不得**移除该关系（目标实际存在，移除反而制造不一致）。
- **情形二/三的结转步骤失败**（原对象仍 `open`、新目标已建立）：原对象**保持 `open`**，本次合并/拆分**未完成**；对原对象**幂等重试**结转（全部目标已存在，关系校验通过），执行前先回读原对象确认尚未关闭。若持续无法完成，则记为未完成范围并如实披露（§15），并须处置已建立的新目标：该目标已是合法的 `open` Spark（本类型**不存在删除操作**，§13），**不得删除或废弃**；经 Human Gate 确认后二选一——① 补做结转完成本次合并/拆分；② 放弃本次合并/拆分，原对象据此重新判断去向（维持 `open` 由 Human 另行处置，或按其自身依据转入其它终态），新目标**保留为独立 Spark**并如实记录其不再承接原对象。不得留下「目标已建立而原对象仍 `open`、两者关系不明」的悬置态。
- **情形三第二步失败**（新目标已建、原对象已结转、某既存目标流水未补）：按情形一第二步失败处置。
- **任何情形均不得**为使两端一致而把任一方改写为虚假内容，也不得在未完成全部步骤时声称合并/拆分已完成。

**关系有效性的前提**：本类型的关系在**全部目标存在且为 `open`** 时有效（§14.1 去向校验）。故关系只在其所属情形的**结转步骤成功后**才成立；未成立时按上述恢复路径处置，不得使原对象长期持有不可解析的关系（否则触发 §17「未完整关闭」的 Stop Condition）。

**并发边界**：本类型不承接并发控制。并发下的同一对象冲突由 03 §9 的 CAS 与单对象基线承担；**不得以内容指纹或单对象 CAS 冒充跨对象并发控制**（03 §9.6 ① ②）。**在跨对象并发冲突检测实际具备之前**，合并/拆分的前提是 Human 确认时不存在对该组 Spark 的并发修改；该前提是否成立按 01 §7.2 由读取时的观察取得，本节不陈述其当前状态。

**声明的自我限制**：本类型**不声明**合并/拆分构成多对象原子变化。凡对外描述或实现记录把它称作「原子」的，与本节不符。
## 10. 来源与证据
...
| Web 列表三联过滤与排序 | Human 浏览 Spark 列表时（10 §5.2、§5.4） | `priority` + `serves` + `status` 三条件筛选；按 `priority` 或时间排序 |
...
- **创建**：C1 提案对象模式（模式内容见重建锚点 §7；其与 04/21 的统一登记由相应来源承接）——AI 只产出提案对象（含查重结果），Human 确认后经受控创建入口落盘；或初始化对话「发现 Gap」时主动创建（此时 goal.md 可能尚未建立，Spark 不声明 serves，与 25 §10「goal.md 缺失时读操作与 Spark 探索不拦」一致）。创建前必须查重。机械校验：字段闭集合法、question 单句可读、scope_boundary 完整、serves（若声明）匹配 goal.md 存在的 SG-n、载体内聚（question/scope_boundary/summary 逐字）。创建后精确回读。
...
- **更正**（终态与 open 皆可）：修正写错、超限或与事实不符的既有内容。与「更新」同入口、同纪律（§9.2 终态不可重开但内容可更正）；**不得借更正改变 `status`**，每次恰好一条 `change_log` 据实说明更正了什么。
...
- 终态处置完整性：进入终态时 disposition 非空；且 disposition 长度不超过 200 字符（§9.1）。
- **载体内聚**：`question`/`scope_boundary`/`summary` 在正文对应段（调查问题/调查边界/当前理解）逐字出现（frontmatter 是唯一权威文本，正文只展开）。
- **非原子例外次序校验（§9.4）**：三情形次序不同，须先判目标构成——**情形一（全部为目标已存在）原对象先落、目标后落**；**情形二（全部目标新建）目标先落、原对象后落**；**情形三（混合）先建全部新目标、后结原对象**。三情形共同可核验项：写原对象关系时全部目标必须已存在且为 `open`（否则零写入拒绝）；受控创建不携带 `relations`/`disposition`；**未完成全部步骤并各自回读通过前，不得声明合并/拆分已完成**；原对象的终态转换恰好一次、恰好一条 change_log，关系多目标由该次转换一次写入。失败处置按情形区分：**情形一**原对象态与关系保持有效、不得改回 `open`，幂等重补目标备注；**情形二/三**原对象保持 `open`、幂等重试，持续失败须记为未完成范围并处置已建立的新目标（**不得删除**——本类型无删除操作，§13；按 §9.4 恢复路径二选一处置）。任何情形均不得为求一致把任一方改写为虚假内容，不得将该操作描述为「原子」。**并发前提**：跨对象并发冲突检测实际具备之前，合并/拆分的前提是 Human 确认时不存在对该组 Spark 的并发修改（该前提是否成立按 01 §7.2 由读取时的观察取得）。

### 14.2 存量对象迁移

字段契约变更必须声明对存量对象的迁移义务（重建锚点 §6 第 3 条，治 v4 backfill 之痛）：

- **v4 存量 103 个 Spark**：不自动迁入 v5 载体。需经 Human 分批受控迁移，逐份满足：v4 `intent` → v5 `intent`（保留理由与后续方向）；v4 `summary` → v5 `summary`（当前语义快照）；v4 `evolution` → v5 `evolution`（关键转折流水）；v4 `priority` **不迁入**（该字段名在 v4 与 v5 语义不同——v4 的取值与判定纪律未经 v5 复核，迁移时一律以 v5 §8 重新分档或留空，不得直接搬运原值）；v4 `disposition_summary` → v5 `disposition`（终态去向）；v4 `related-to` 关系不迁移为 v5 关系（v5 仅 merged-into/split-into 两键）。
...
| 载体内聚 | 创建与更新时 | `question`/`scope_boundary`/`summary` 在正文对应段逐字出现 | frontmatter 与正文对照 | 机械（逐字比对） | 索引与主体一致性 | 拒绝；补齐后重走 |
...
| priority 分档适当性 | 创建、调整时 | 档位与当前目标、阶段及 `scope_boundary` 相称 | 分档理由（提案或 Human 调整记录） | **AI 综合判断 + Human 可调整** | 当次分档依据 | 由 Human 调整；不作机械拒绝 |
