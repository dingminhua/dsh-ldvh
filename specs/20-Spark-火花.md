...
ldvh_spec:
  ...
  relation: "refines"
  positioning: "定义 Spark 事实类型的目的、对象边界、字段、状态、悬置语义、合并/拆分关系与终态处理，是 WC 授权层 Gap 的调研门扉"
  scope: "适用于管辖项目中承载悬置问题的 Spark 对象；不定义调研执行流程（交给 11 号调研系统）、不定义 WorkCase 授权、也不定义 Initiative 专项"
  basis:
...
Spark 解决工作缺乏锚点、目标不清晰、外部信息缺失等悬置问题的持续成本：跨会话未收敛信息反复丢失、重复讨论、被过早升级为行动或决定，以及无法明确退出入口。Spark 以悬置层承载这些内容：目标已定（那是 25 号 Goal），不是悬而未决；它承接「尚未形成确定承接位置」的单一信息需求、发现、问题或缺口，供后续召回、查重、分流或深挖。普通文件或 README 不能无损承载：它们无受控写入、无 change_log、无状态闭集、无 Gate 1 引用契约。Spark 填补 WC 授权层 Gap 与调研系统之间的跨层桥梁：调查问题 → 调研报告 → 交付决策。
...
11 号是调研流程的唯一权威；21 号是 WorkCase 授权语义的唯一权威；25 号是 Goal/sub-goal 锚点的唯一权威；24 号是 Research 事实类型的唯一权威。本文只定义 Spark 类型自身的悬置语义、字段、状态、关系与终态，不复制相邻规范的格式、协议或实现细节。
...
明确排除：调研执行（交给 11 号）；答案收集（调研系统）；结论落地（WorkCase）；Initiative 组织层（26 号不存在）；已定目标的承载（25 号）。Spark 对象进入调研系统后不能回到 Spark 继续悬置。Spark 对象存在、处于 open 状态或终态，均不能单独证明问题真实存在、值得调查、调查已完成或下游工作已承接——这些须由消费点与 Human 决定共同核对。

## 5. 类型定位与价值

Spark 是悬置层的事实类型：承载「尚未形成确定承接位置」的单一信息需求、发现、问题或缺口，是候选议题的低摩擦承载位。它横切全部层级——Goal 层方向疑问、WC 层执行发现的悬置、讨论残项的显式悬置（12 号 §10.2）。

与 24 号的最大差异：Spark 不调查外部世界，只承载内部未决问题；调研的执行由 11 号承接。与 25 号的最大差异：Goal 是已定目标的冻结锚，Spark 是未定之事的悬置层。

## 6. 对象边界

### 6.1 什么构成一个对象

一个 Spark 对象 = 一个可独立判断的悬置问题（question）+ 明确的调查边界（scope_boundary）+ 保留理由与后续方向（intent）+ 完整的当前语义快照（summary）+ 演变流水（evolution）。问题必须可独立召回、可独立判断、可独立分流。
...
以下内容不单独形成 Spark：当前行动可直接处理且不需跨会话保留的内容；已经由现有位置完整承载的内容；已经具有明确行动目标的内容（走 WorkCase）；已经形成长期决定或复用经验的内容（走 ADR/Pitfall）；完整研究报告（走 Research）；过程日志、逐条聊天、工具输出与单纯执行提醒。
...
悬置问题来源于 Human 决策（WC 授权 Gap 时的悬置表达、初始化对话的发现、讨论残项的显式悬置）。Spark 本身不直接接受外部证据——由 11 号调研系统收集；本类型不设 `urls` 字段（与 24 号的最大差异）。
...
| 消费点 | 时机 | 消费内容 |
|---|---|---|
| 11 号调研系统 | 悬置问题转入调研时 | question + object_uid |
| Spark 讨论 | 每次扩散前 | question + scope_boundary + summary |
| 讨论系统残项悬置 | 未收敛点分流时（12 号 §10.2） | 未决问题 + 判断标准与监测条件 |
