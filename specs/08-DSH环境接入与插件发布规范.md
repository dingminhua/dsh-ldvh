...
| `ctx.tools.guard` | 经该入口注册单调守卫 | 不得主张执行链对 LDVH 操作有守卫约束 |
| `ctx.invariants.register` | 经该入口部署运行时不变量断言 | 不得主张本节所述检查时机对 LDVH 操作生效 |
| `fs/observed` 读观察 | 挂载读观察并挂载写入版本守卫 | 不得主张版本或指纹屏障已生效 |
| `ctx.userQuestions.ask` | 经该入口承载 Human Gate 决策提请 | 不得主张问询已经该入口 |
