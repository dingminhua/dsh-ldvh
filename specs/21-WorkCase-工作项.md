...
| `serves` 与 20/26 号 `serves_sg` 命名不一致 | 同段类型规范的同类字段命名分裂（本文与 25 号同源用 `serves`） | 03 §7.2 已就此登记命名纪律：统一须**规范侧（20/26）+ 实现侧字段闭集 + 既有对象迁移**三方同步，完成前各类型现行登记名继续有效；本文的 `serves` 与 25 号一致，不因该分裂失效 |
