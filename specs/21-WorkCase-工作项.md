...
3. 子代理 spawn、通信、中断与结果回收的宿主机制（归 02 §8 与 08）；attempt 的宿主实现与锁语义由 08/09 承接，本文只定义类型侧字段与不变量；
...
- `open → draft`（**C2 局部重批**）：执行中 `plan` 或 `scope` 发生实质变化，使 `authorization_fingerprint` 不再匹配时，授权在受影响范围自动失效并回到待批准；`attempt` 作废（不续跑），已取得的 `result` 证据保留为历史记录；重批只针对受影响范围，不整单重走。
...
4. attempt 的宿主实现、锁语义与接入由 08/09 承接（02 §18 明示 02 不预设其语义）；本文只定义对象上必须存在的字段、不变量与恢复判据。
