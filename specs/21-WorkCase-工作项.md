...
WorkCase 是工作执行业务系统（00 §3.1、02 §12）的结论承载：它把「执行已批准的工作」这一职责落成可回读的对象，但价值的兑现仍依赖讨论系统提供格式齐全的计划（12 号）、遵守系统落实适用约束（02 §13）与复核制度对照实际结果（02 §15）；本文只定义类型自身的边界、字段、状态、授权语义与消费点，不替代上述相邻职责。
...
4. `discussion-system-foundation`：12 §5 需求审核职责前置与 WorkCase 只承担执行与结果审计、§10.1 四路分流中 WorkCase 出口的成文格式权威、§10.2 残项处置。

本文的结构父规范是 `fact-model-foundation`（03），按 01 §10.1 同一目标不得同时作为 `basis` 和结构父规范，故 03 不列入 `basis`；03 §11 定义的 20–24 事实类型规范共同结构是本文 §5–§15 的结构依据，与 20/22–26 各同段类型规范同形态。
...
1. 需求本身的裁夺与「做什么」的决定（归 Human 意图与 goal，12 §5）；
2. 方案构建、扩散与收敛流程（归 12 号讨论系统）；**需求审核阶段已由 12 号前置收敛机制砍除，WorkCase 不承担该阶段**；
3. 子代理 spawn、通信、中断与结果回收的宿主机制（归 02 §8 与 08）；attempt 的宿主实现与锁语义**指定由 08/09 承接**（**尚未实装**——08/09 当前无 attempt 内容）；本文只定义类型侧字段与不变量；
...
03 是公共字段与受控读写契约的唯一权威；06 是事实源与受控提交的唯一权威；25 是 Goal 与 sub-goal 锚点的唯一权威；23 是 Pitfall 类型语义的唯一权威；12 是方案收敛与四路分流的唯一权威；08/09 被指定为 attempt、锁与冷恢复宿主实现的承接方（**该承接尚未实装**，见 §10.4 第 4 项与 §15 缺口登记）。**本文是 WorkCase 类型语义以及 Gate 1 与 Gate 2 授权语义的唯一权威**：Gate 1 授权模板、授权包结构、C2 钉扎与局部重批语义只在本类型规范中定义，不复制相邻规范的规则。10 §6.2 将 Gate 1 / Gate 2 标注为「WorkCase provisional 术语，在 21/34 定稿前不作为触发依据」，其 Gate 语义部分随本文定稿而不再 provisional；34 号属目标与蓝图业务系统段，不定义事实类型的状态机与授权语义，本文不把 Gate 1 / Gate 2 的语义分给 34 号（该冲突点已在交付报告中登记，引用同步不由本文执行）。
...
明确排除：方案从模糊到可执行的构建过程（归 12 号）；方向未定或未决问题（归 20 号 Spark）；已定决策本身（归 22 号 ADR）；外部对象深度调研（归 24 号 Research）；项目目标本身（归 25 号 Goal）；未形成方案的临时行动（不对象化）。WorkCase 对象存在、Gate 1 记录齐全、attempt 有效或状态为 open，均不能单独证明计划正确、授权仍覆盖当前动作、执行正在推进、结果已达成、残留责任已交接或价值成立。

## 5. 类型定位与价值

WorkCase 是授权执行层的事实类型：**已批准工作及其结果审计的唯一承载**。它把一次工作从「对话里的计划」变成可回读、可拒绝越权、可中断恢复、可逐条核对的对象。它只承担两件事——**执行与结果审计**（12 §5：需求审核职责已前置到讨论，本文据此不设需求审核阶段或状态）。
...
| `plan` | array | 必填 | 可执行计划步骤 | 每项 `{step, done_criteria}`；`done_criteria` 必须可被证据判定；格式齐全只待批准（12 §10.1 出口形态） |
...
状态闭集三值：`draft`（计划已成形、待 Gate 1 批准）→ `open`（Gate 1 已批准、执行中）→ `closed`（终态，配 `outcome` 四值之一）。本文不设「需求审核」状态或阶段——该职责已由 12 §5 前置到讨论收敛，WorkCase 只承担执行与结果审计。
...
结果审计是 WorkCase 的固有职责（12 §5），形态是 `result.criteria_checks` 对 `plan[].done_criteria` 的逐条核对：
...
4. **宿主实现待补（如实声明）**：attempt 的宿主实现、锁语义与接入**应由 08/09 承接**（02 §18 明示 02 不预设其语义），但**截至本文定稿，08 与 09 均尚无 attempt 相关内容**（本仓库检索 `attempt` 在 08/09 规范中零命中）。因此本文定义的 attempt 字段契约（单调 `attempt_id`、至多一个活跃、孤立 attempt 判定、`heartbeat_at` 刷新）**当前没有可执行的宿主机械保障**；V6 工作接续与 HV2 授权执行受控可续中依赖 attempt 的部分，在 08/09 补齐前不成立。本项作为跨规范接缝缺口登记，不得表述为已由 08/09 承接；
5. attempt 存在、`heartbeat_at` 刷新或子代理返回，均不证明执行正在推进、已获授权、结果已达成或工作已完成。

## 11. 来源与证据

计划的来源是讨论收敛产物（12 §10.1）或 Human 直接提出的已定工作；结果的来源是当次执行的实际观察、写后回读与 Git 状态。本类型不设 `urls`：外部长期资料不进入 WorkCase，必要时在正文自然语言交代并由 `relations` / F4 展开指向 22/24 号对象。
...
| 消费点 | 时机 | 消费内容 |
|---|---|---|
| 12 号四路分流出口 | 收敛产物判定为可执行计划时（12 §10.1） | 计划 + 完成判据 + `serves`；成文格式权威是本文 |
...
**与 12 号的交接契约**：讨论收敛产物进入 WorkCase 时，12 §10.1 保证「格式齐全、只待 Gate 1 批准」，即 `summary`、`scope`、`plan`（含逐条 `done_criteria`）与 `serves` 四项可填且未收敛点已显式分流；本文据此不设需求审核阶段——方案构建的拉锯不进入 WorkCase 状态机。交接缺失任一项时，工作包回到 12 号补收敛，不得以 draft 状态长期承载「还在变的方案」。
...
### 15.3 未实现范围（gaps，如实登记）

| 缺口 | 影响 | 实现前的替代保障 |
|---|---|---|
| attempt 宿主实现（§10.4 第 4 项） | 类型侧 attempt 契约无宿主机械保障；V6 接续与 HV2 受控可续中依赖 attempt 的部分不成立 | 无。在 08/09 补齐前，不得声称这些价值已兑现 |
| 34 号引用（10 §6.2「21/34 定稿前」） | 本文已定稿，但 34 号尚不存在；10 §6.2 的 provisional 措辞未随之更新 | 本文 §3.3 已声明 Gate 语义随本文定稿不再 provisional；10 号措辞更新由 10 号侧执行 |
| `serves` 与 20/26 号 `serves_sg` 命名不一致 | 同段类型规范的同类字段命名分裂（本文与 25 号同源用 `serves`） | 命名是否统一由 03 侧公共契约决定；本文暂与 25 号保持一致 |

**本节不得因上述缺口被改写为「已承接」或「已保障」**；缺口闭合后应回填对应行并移除。
...
|---|---|---|---|---|---|---|
| 计划可判定性 | 创建或局部重批时 | `plan` 每项 `done_criteria` 非空且可被证据判定 | 计划全文 + AI 逐条自检 | AI 语义审核 + Human 确认 | 当次计划判据的可判定性 | 拒绝创建或回到 12 号补收敛 |
...
1. 不恢复需求审核阶段或状态——该职责已由 12 号前置收敛机制承接；
