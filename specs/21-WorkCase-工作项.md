...
4. **宿主实现未覆盖时的声明限制**：attempt 的宿主实现、锁语义与接入**应由 08/09 承接**（`02 §18` 明示 02 不预设其语义）。**该承接落地之前**：本文定义的 attempt 字段契约（单调 `attempt_id`、至多一个活跃、孤立 attempt 判定、`heartbeat_at` 刷新）**没有可执行的宿主机械保障**，且 V6 工作接续与 HV2 授权执行受控可续中**依赖 attempt 的部分不成立**；**不得表述为已由 08/09 承接**。本项属跨规范接缝缺口，其当前状态登记于 `docs/audit/`（无规范效力）。
