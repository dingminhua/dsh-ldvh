...
内容区：
  ...
  Spark：意图 / 摘要 / 演变 / 落实或废弃（终态时）/ 关联（存在时）
  其他对象：字段卡片布局
...
6. **当前复核、授权边界与批准是什么**：活动期读取 `plan_version`、当前 `creation_reviews`、`execution_authorization`、`execution_approval`，以及已经形成时的 `result_version`、`controller_check_summary` 和当前 `result_reviews`。详情必须分别呈现 Reviewer 复核、Controller 处置、执行授权边界与 Human 执行批准；每条 review 显示实际方法，same-AI fallback 还必须显示 limitation 引用、当次能力证据、assurance gap 与停止条件评估，绝不标成独立 subagent。授权边界固定形成“已授权行动”“禁止行动”、实际存在时的“Human 前置条件”和“能力限制”并列、默认收起的数量区块；能力限制展开后完整显示 capability/availability、观察与基线证据、受影响类别、fallback policy、assurance gap 和停止条件。“已授权行动”展开后每项动作也默认收起，仅以“动作摘要＋稳定 ID”作为入口；打开单项后才显示目标/影响范围，风险、回滚与规则引用仍是次级展开内容，规则引用使用标签而非项目符号。行动上限、允许调整、验证回滚和越界处置作为“授权约束”置于同一个“已授权行动”展开区的底部。批准必须显示绑定版本、`baseline_fingerprint` 与非空 `source_refs`。
...
当前 `WorkCaseReadingLayout` 已直接按 21 单一当前字段族实现以下固定节点顺序：当前情况 → 目标 → 成功标准 → 当前计划与工作项 → 方案复核 → 执行授权边界 → 执行批准 → 结果与验证 → 主控自检 → 结果复核 → 关闭提案 → 终态处置 → 关联 → 外部网址。“目标”节点内依次呈现 `goal` 与作为次级字段的 `scope`。节点只按对应字段实际存在显示；类型来源必填字段缺失或字段类型不符时，对应节点仍然保留，并在该字段位置就地标明字段问题（与其它事实类型共用同一套 `fieldIssue` / `FieldProblem` 组件），不升级为整对象读取失败，也不只依赖页底汇总。正文完整渲染、不截断：目标、当前情况、标准、工作内容、结果和处置说明是主阅读信息；预期结果、方法边界、验证说明、复核方法/保证边界、复核反馈、主控处置、剩余责任和恢复条件按职责形成次级语义块。计划版本与结果版本进入节点标题栏；Reviewer 身份、事件时间和结论进入记录标题或弱元信息，不再与正文排成同权值对。工作项依赖属于机器执行约束，不进入 Human 阅读节点，但仍保留在来源字段、API 和 YAML 数据中。Human 执行批准、Reviewer 复核、Controller 处置、关闭提案和终态处置仍保持独立节点，职责提示只解释边界，不替代事实。关闭提案中的拟路由目标按关联区同一规则处理：同项目目标按需解析当前目标标题并可进入右侧扩展阅读，精确读取后提供对象路径复制；项目身份与内容 fingerprint 降权保留为次级定位信息；跨项目或身份不完整的目标只如实显示已知稳定身份，不以 object_id 冒充名称。成功标准定义与三值结果按 `criterion_id` 对照；`relations` 的正式 kind 继续保留在来源、详情 API 与折叠 YAML 中，Human 关联区则与其它事实对象统一为“类型分组＋目标标题＋小号状态＋扩展阅读入口”，不显示目标 ID 或复制按钮，也不另加 WorkCase 专属关系芯片。详情与右侧扩展阅读复用同一组件和同一投影，不存在历史字段、列表回退或状态分支。
...
“方案复核”和“结果复核”的每条记录先在 Reviewer 身份与结论标题栏下显示实际方法；same-AI fallback 使用明显的低保证信息面，完整显示 limitation、当次 evidence、assurance gap 和 stop assessment。随后固定形成三个同级内部框：灰色“复核范围”、琥珀色“复核反馈”和青色“主控处置”。三个内部框标题均不放圆点，只保留标题文字与右侧折叠箭头；复核范围默认收起，复核反馈和主控处置作为当前判断默认展开。圆点只用于反馈框内部的并列反馈项，使用 `4px` 圆点并与正文首行中线对齐；反馈项正文移除 Markdown 默认段落外边距，只保留条目之间 `6px` 的列表间隔。不得把复核范围恢复为无框裸文本，也不得把 Reviewer 反馈与 Controller 处置合并为同一判断。
...
| Spark | 意图 `intent` → 摘要 `summary` → 演变 `evolution` → 关联 | `implemented` 显示“落实”、`discarded` 显示“废弃”；两者内容均只读 `disposition_summary` 与 `updated_at`。历史 `routed` 只作只读兼容，不参与当前终态投影 | 用旧 `source_detail`、`description`、`resolved_to`、`resolved_at`、`discard_reason` 字段，或在 `open` 状态制造终态占位 |

- ADR 的 `retired` 中文状态显示为”已废弃”；Study 的 `retired` 中文状态显示为”已废弃”；Pitfall 状态显示 `draft / active / discarded`，分别使用“待确认 / 活跃 / 已废弃”。Pitfall 的 draft、discarded 仍使用与 active 相同的完整正文阅读形状；discarded 的处置说明 Human 未接受 draft，或曾经 active 的经验已不再适用及其去向，不生成额外时间字段。详情页不提供 promote、discard、批量审核或自动过期控件。
...
关联区块内每个具体条目必须使用统一行结构：左侧为语义图标和标题/路径/网址文本，右侧提供扩展阅读入口。正式事实关联不显示目标 ID 或复制入口；文档路径与外部 URL 仍按其来源能力提供复制入口。整行必须可点击并触发扩展阅读；复制入口只执行复制，不触发扩展阅读。对象引用、文档路径和外部 URL 不得各自使用不同的卡片、文字按钮或标签样式。行内语义图标、标题文本、复制入口和扩展阅读入口必须垂直居中；复制和扩展阅读入口使用同一 28px 操作容器，长标题截断或出现摘要次行时也不得让右侧操作图标下坠或上浮。
...
Spark 不使用普通字段卡片堆叠。Spark 是“待处理信息对象”，页面目标是说明当前信息如何继续被理解或已经如何处置，而不是证明结论、沉淀经验或呈现报告。意图读取 `intent`，摘要读取 `summary`，演变读取 `evolution` 并以倒序的 `at + summary` 短卡片显示。`open` 不显示终态节点；`implemented`、`discarded` 分别显示“落实”“废弃”，都只读取 `disposition_summary` 和对象 `updated_at`。历史 `routed` 只可按历史基线读取，不提供为当前终态的展示、筛选或操作。Web 只读展示，不提供状态修改、落实或废弃按钮，也不从终态处置推断被关联对象的完成状态。
