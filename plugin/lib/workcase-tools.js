...
function parameterSchemaFor(operationKey) {
  ...
  const workcaseFrontmatter = {
    ...
    properties: {
      ...
      summary: { type: "string", description: "要做什么的当前语义快照（AI 向，不设长度上限）；使未读原计划的后续执行者可独立执行；必须逐字出现在「摘要」正文节。**书写格式（21 §8，机械校验）**：超过 200 字符时须用空行分块，且**首块之外**的每块首行须是层次标记行——`**标签**`、`### 标签` 或 `标签：`（如 `**现状核实**` / `**处置方向**` / `**边界**`）。首块可为总述。标签名由你按内容定，规则只要求「有标记、看得出这一块在讲什么」；200 字符以内不强制分块" },
      scope: { type: "string", description: "授权范围与边界：必须同时回答「做什么」与「明确不做什么」；是越权拒绝的比对基准；必须逐字出现在「授权范围」正文节。**书写格式（21 §8，机械校验）**：「做什么：」与「明确不做什么：」必须**各自独占一行**并各带内容（不得写成 `做什么：A；B。明确不做什么：C。` 这类行内串联）；两者下的具体条目用 `- ` 列表逐条列出。该骨架不受长度豁免——任何长度都须两段分明；超过 200 字符时另须用空行分块（同 summary）" },
      plan: {
        type: "array",
